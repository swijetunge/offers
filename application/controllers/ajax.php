...
require_once $_SERVER['DOCUMENT_ROOT'].'/application/views/includes/config.php';

class ajax extends controller
{
	public function __construct() {
		
		define( 'BASE_PATH', 'http://local.test/');
		...
		if( isset($_POST['type']) && !empty( isset($_POST['type']) ) ){
			...
			switch ($type) {
				case "save_user":
					$this->save_user($mysqli);
					break;
				case "delete_user":
					$this->delete_user($mysqli, $_POST['id']);
					break;
				case "getUsers":
					$this->getUsers($mysqli);
					break;
				default:
					$this->invalidRequest();
			}
	...
	function save_user($mysqli){
		try{
			$data = array();
			echo $name = $mysqli->real_escape_string(isset( $_POST['user']['name'] ) ? $_POST['user']['name'] : '');
